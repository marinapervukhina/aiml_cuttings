"""
NAME : 
    model_training_min100px.py

DESCRIPTION

    This script will create  the network and training it using training and validation datasets for images with size greater or equal than 100px.
    The structure of the code is the same as model_training_flow.py, but the network is different. 
    It uses ImageDataGenerator flow_from directory to create the dataset, the root folder must have the images organised by class. For example:
    Images_min55px:
    |__training_dataset
        |__carbonates
        |__mudstone
...
#Import Matplotlib to  visualise metric
...
def create_datagen():
    ...
    train_datagen =ImageDataGenerator(
        rescale=1. / 255,
    #add prepocess here  
        #brightness_range = [0.8, 1.2], # 1.0 as it si, <1, dark
        #zoom_range = 0.2,
